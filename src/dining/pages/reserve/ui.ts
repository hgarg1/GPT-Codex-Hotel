...
import type { DiningTable, ReserveState, ReserveStep } from './types.js';

const root = document.getElementById('dining-reserve-root');
const alertsRegion = document.getElementById('dining-reserve-alerts');

let state: ReserveState = loadState();
let holdCountdownInterval: number | null = null;
...
function showAlert(message: string, tone: 'info' | 'success' | 'warning' | 'danger' = 'info'): void {
...
function ensurePreviousSteps(): ReserveState {
...
async function ensureTablesLoaded(): Promise<void> {
...
function getStepperMarkup(currentStep: ReserveStep): string {
  const steps: ReserveStep[] = ['schedule', 'party', 'seats', 'guest', 'review'];
  return `
...
function renderScheduleStep(): string {
...
function renderPartyStep(): string {
...
function tableStatus(table: DiningTable): 'selected' | 'available' | 'held' | 'unavailable' {
...
function renderSeatMap(): string {
  if (state.tables.length === 0) {
    return '<p class="reserve-muted">Loading seating plan…</p>';
  }

  const [minX, minY, maxX, maxY] = state.tables.reduce(
    (acc, table) => {
...
function renderSuggestedCombos(): string {
...
function renderSeatsStep(): string {
...
function renderGuestStep(): string {
  const { guest } = state;
...
function renderConfirmationStep(): string {
...
function getSelectedCapacity(): number {
...
function attachHandlers(): void {
  ...
  const backButton = root.querySelector('[data-action="back"]') as HTMLButtonElement | null;
  if (backButton) {
  ...
  if (state.step === 'schedule') {
    const form = document.getElementById('reserve-schedule-form') as HTMLFormElement | null;
  ...
  if (state.step === 'party') {
    const form = document.getElementById('reserve-party-form') as HTMLFormElement | null;
    form?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const formData = new FormData(form);
  ...
  if (state.step === 'seats') {
    ...
    if (seatMap) {
      seatMap.addEventListener('click', (event) => {
        ...
        const button = target.closest('button[data-table-id]') as HTMLButtonElement | null;
        if (!button) return;
    ...
    const comboButtons = root.querySelectorAll<HTMLButtonElement>('[data-action="apply-combo"]');
    ...
    const continueButton = root.querySelector('[data-action="continue-seats"]') as HTMLButtonElement | null;
  ...
  if (state.step === 'guest') {
    const form = document.getElementById('reserve-guest-form') as HTMLFormElement | null;
    form?.addEventListener('submit', async (event) => {
      event.preventDefault();
      if (!state.hold || !state.hold.holdId) {
  ...
  if (state.step === 'review') {
    const confirmButton = root.querySelector('[data-action="confirm"]') as HTMLButtonElement | null;
  ...
  if (state.step === 'confirmation') {
    const button = root.querySelector('[data-action="new-reservation"]') as HTMLButtonElement | null;
    button?.addEventListener('click', () => {
      state = resetState();
      saveState(state);
      renderStep();
...
function refreshHoldCountdown(): void {
...
async function refreshAvailabilityOnFocus(): Promise<void> {
  if (!state.date || !state.time || !state.partySize) {
    return;
  }
  try {
...
export async function initializeReserveUI(): Promise<void> {
  if (!root) {
